...
const getPage = async (question, request, h) => {
  ...
  if (url === 'check-details') {
    ...
    const agentAddress = saveValuesToArray(agentDetails, ['address1', 'address2', 'county', 'town', 'postcode'])

    const contractorContact = saveValuesToArray(contractorDetails, ['emailAddress', 'mobileNumber', 'landlineNumber'])
    const contractorAddress = saveValuesToArray(contractorDetails, ['address1', 'address2', 'county', 'town', 'postcode'])

    const farmerContact = saveValuesToArray(farmerDetails, ['emailAddress', 'mobileNumber', 'landlineNumber'])
    const farmerAddress = saveValuesToArray(farmerDetails, ['address1', 'address2', 'county', 'town', 'postcode'])
    const MODEL = {
      ...question.pageData,
      backUrl,
      nextUrl,
