...
describe('Score page', () => {
  ...
  const getProdScoringSpy = jest
    .spyOn(newSender, 'getProdScoring')
    .mockImplementation(() => {
      Promise.resolve(scoreDataRobotics)
    })
  const getDesirabilityAnswersSpy = jest
    .spyOn(createMsg, 'getDesirabilityAnswers')
    .mockImplementation(() => {
      return {
        test: 'test'
      }
  ...
  it('should load page with success Robotics Strong', async () => {
    ...
    crumCookie = getCrumbCookie(response)
    expect(response.result).toContain(crumCookie[1])
    const responseScoreMessage =
      'This means your project is likely to be successful.'
  ...
  it('should load page with success Robotics Average', async () => {
    ...
    const scoreSpan = page.querySelectorAll('span.govuk-heading-m')
    expect(scoreSpan.length).toBe(1)
    expect(extractCleanText(scoreSpan[0])).toBe(scoreText)

    const scoreParagraph = page.querySelectorAll('p')
    const scoreMessage = getTargetByText(scoreParagraph, responseScoreMessage)
  ...
  it('should load page with sucess Robotics Weak', async () => {
    ...
    const scoreText = 'Weak'
    const responseScoreMessage =
      'This means your project is unlikely to be successful.'
    ...
    const page = createPage(response.payload)
    const scoreParas = page.querySelectorAll('p')
    const scoreSpan = page.querySelectorAll('span.govuk-heading-m')
    expect(scoreSpan.length).toBe(1)
    expect(extractCleanText(scoreSpan[0])).toBe(scoreText)

    const scoreMessage = getTargetByText(scoreParas, responseScoreMessage)
    expect(scoreMessage.length).toBe(1)
  ...
  it('redirects to project business details page', async () => {
    ...
    expect(postResponse.headers.location).toBe(
      `${global.__URLPREFIX__}/business-details`
    )
  })
})
